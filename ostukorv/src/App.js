...
function App() {
  const [ostukorv, setOstukorv] = useState([
    { nimetus: 'pirnid', kogus: 1, yhik: 'kg', korvis: false},
  ...
  const lisaOstukorvi = () => {
    ...
    //ostukorv.push(uusToode) nii ei saa lisada et reaktiivsus säiliks
...
const toggleKasKorvis = (index) => {
  ...
  return (
    <div className="container">
      <div className='row'>
      <input className='col-4' value={toode} onChange={toodeMuutus} placeholder='Toode'/>
      <input className='col-2' value={kogus} 
      onChange={(event) => setKogus(event.target.value)} 
      placeholder='kogus'
      />
      <input className='col-2' value={yhik} 
